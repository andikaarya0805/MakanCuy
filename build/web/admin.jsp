...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .sidebar { 
            width: 260px; background: var(--sidebar); border-right: 1px solid #333; 
        ...
        table { width: 100%; border-collapse: collapse; min-width: 600px; /* Biar gak gepeng di HP */ }
        ...
        /* GRID LAYOUT FOR MENU TAB */
        .menu-grid { display: grid; grid-template-columns: 2fr 1fr; gap: 20px; }

        /* --- MOBILE RESPONSIVE --- */
        @media (max-width: 768px) {
            .sidebar { 
                position: fixed; left: -260px; top: 0; bottom: 0; height: 100vh;
            ...
            /* Hide columns in small screens if needed, or rely on scroll */
...
<body>
    ...
    <script>
        ...
        function switchTab(name) {
            ...
            // Tutup sidebar otomatis di HP pas ganti tab
            ...
            try {
                ...
                if (!data.history || data.history.length === 0) {
                    ...
                    data.history.forEach(item => {
                        ...
                        // Logic Badge & Actions
                        if (item.status === 'PAID') {
                            statusBadge = '<span class="status-badge status-paid">Konfirmasi</span>';
                            actionButtons = 
                                '<div style="display:flex; gap:5px;">' +
                                    '<form action="admin" method="POST">' +
                                        '<input type="hidden" name="action" value="confirm">' +
                                        '<input type="hidden" name="id" value="' + item.id + '">' +
                                        '<button type="submit" class="btn" style="background:#4ade80; color:#000; padding:5px 8px; font-size:0.75rem;">✔</button>' +
                                    '</form>' +
                                    '<form action="admin" method="POST">' +
                                        '<input type="hidden" name="action" value="reject">' +
                                        '<input type="hidden" name="id" value="' + item.id + '">' +
                                        '<button type="submit" class="btn" style="background:#f87171; color:#fff; padding:5px 8px; font-size:0.75rem;">✖</button>' +
                                    '</form>' +
                                '</div>';
                        } else if (item.status === 'PROCESSING') {
                            statusBadge = '<span class="status-badge status-process">Proses</span>';
                            actionButtons = '<span style="color:#4ade80; font-size:0.9rem;">✔ OK</span>';
                        } else if (item.status === 'REJECTED') {
                            statusBadge = '<span class="status-badge status-reject">Ditolak</span>';
                            actionButtons = '<span style="color:#f87171; font-size:0.9rem;">✖ NO</span>';
                        } else {
                            statusBadge = '<span class="status-badge" style="color:#888;">' + item.status + '</span>';
                            actionButtons = '-';
                        }
                        ...
                        tbody.innerHTML += 
                            '<tr>' +
                                '<td>#' + item.id + '</td>' +
                                '<td><span style="white-space:nowrap; font-size:0.8rem">' + item.date + '</span></td>' +
                                '<td>' +
